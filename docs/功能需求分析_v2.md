...
| 功能 | 需要新表格 | 優先級 | 複雜度 |
|------|-----------|-------|-------|
| 使用者自行新增幣種 + 管理者審核 | ✅ `coin_submissions` | 高 | 中 |
...
### 1. coin_submissions（幣種申請表）

#### 功能說明
- **使用者側**：可以提交新幣種申請（平台沒有的幣種）
- **管理員側**：審核申請，決定是否通過
- **流程**：提交 → pending → 管理員審核 → approved/rejected

#### 資料表結構
```sql
CREATE TABLE coin_submissions (
    id                      BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id                 BIGINT UNSIGNED NOT NULL,       -- 提交者
    coin_symbol             VARCHAR(20) NOT NULL,           -- 幣種代號（BTC, ETH）
    coin_name               VARCHAR(100) NOT NULL,          -- 幣種全名
    description             TEXT NULL,                      -- 描述
    website_url             VARCHAR(255) NULL,              -- 官網
    whitepaper_url          VARCHAR(255) NULL,              -- 白皮書
    coingecko_id            VARCHAR(64) NULL,               -- CoinGecko ID
    status                  ENUM('pending', 'approved', 'rejected') DEFAULT 'pending',
    reviewed_by_user_id     BIGINT UNSIGNED NULL,           -- 審核者（管理員）
    review_note             TEXT NULL,                      -- 審核意見
...
#### 前端頁面需求
1. **使用者頁面**：「提交新幣種」表單
   - 輸入：幣種代號、名稱、描述、官網、白皮書
   - 顯示：我的申請列表（狀態：待審核/已通過/已拒絕）
...
#### API 端點設計
```
POST   /api/coin-submissions          # 提交申請
GET    /api/coin-submissions/my        # 查詢我的申請
GET    /api/coin-submissions/pending   # 管理員查詢待審核（需 admin 權限）
PATCH  /api/coin-submissions/:id       # 管理員審核（approve/reject）
```
...
2. **`CoinSubmissionView.vue`** - 提交新幣種
   - 申請表單
   - 我的申請列表
...
3. 重要 API：
   ...
   - `CoinSubmissionService.reviewSubmission(id, approved, note)` - 審核申請

### 前端 (Vue 3)
1. 建立新工具函數：
   - `src/utils/transactions.js` - 交易記錄 API
   - `src/utils/submissions.js` - 幣種申請 API
...
### 資料庫變更
- **新增 3 個表格**：`coin_submissions`, `coin_comparisons`, `user_transactions`
- **原有表格**：無變更（100% 向下相容）
- **資料遷移**：無需遷移
